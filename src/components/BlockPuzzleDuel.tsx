...
export const BlockPuzzleDuel: React.FC = () => {
  ...
  const mapServerBlockPuzzleMatches = useCallback((matches: any[]): PendingMatchItem[] => {
    return (matches || []).map((m: any): PendingMatchItem => {
      const outcome = m.outcome === 'WON' ? 'WON' : m.outcome === 'LOST' ? 'LOST' : m.outcome === 'DRAW' ? 'DRAW' : 'PENDING';
      return {
        id: String(m.id),
        userId: String(m.userId || user?.id || ''),
        userName: String(m.userName || user?.name || ''),
        entryFee: Number(m.entryFee || 0),
        prize: Number(m.prizeAmount || 0),
        score: Number(m.score || 0),
        linesCleared: Number(m.linesCleared || 0),
        bestCombo: Number(m.bestCombo || 0),
        date: m.createdAt ? new Date(m.createdAt).toLocaleString('en-GB') : '',
        status: m.status === 'REFUNDED' ? 'LOST' : (m.status === 'COMPLETED' ? outcome : 'PENDING'),
        mode: m.tournamentId ? 'Tournament' : 'Online Duel 1v1',
        opponentName: m.opponent?.name || undefined,
        opponentScore: m.opponent?.score ?? undefined,
      };
    });
  }, [user?.id, user?.name]);

  const refreshBlockPuzzleMatches = useCallback(async () => {
    const matches = await getMyBlockPuzzleMatches();
    setPendingMatches(mapServerBlockPuzzleMatches(matches));
  }, [getMyBlockPuzzleMatches, mapServerBlockPuzzleMatches]);
  ...
  return (
    <div className="min-h-screen bg-[#070b18] text-white flex flex-col justify-between max-w-md mx-auto relative select-none">
      ...
      {screenState === 'matchmaking' && (
        <div className="flex-1 flex flex-col justify-between p-5 text-center relative overflow-hidden animate-fade-in">
          ...
          <div className="space-y-6 my-auto">
            ...
            <div className="bg-[#0b1028]/80 p-4 rounded-3xl border border-indigo-900/60 border-dashed max-w-xs mx-auto text-center space-y-1">
              ...
              <span className="text-[10px] text-slate-300">
                At match end, submit your score for Admin review
              </span>
            </div>
          </div>

          {/* Waiting status: no manual start. Server starts both players together. */}
          <div className="space-y-3 max-w-xs mx-auto w-full">
            ...
            <div className="text-[11px] text-cyan-300 font-bold animate-pulse">অপর প্লেয়ার পাওয়া গেলে ম্যাচ স্বয়ংক্রিয়ভাবে শুরু হবে…</div>
          </div>
        </div>
      )}
      ...
      {screenState === 'playing' && (
        <div className="pb-28 pt-2 px-3 space-y-3 relative flex flex-col flex-1">
          ...
          <div className="relative z-30 w-full max-w-[390px] mx-auto px-3 py-2 bg-[#070b18]/95 backdrop-blur-md rounded-2xl border border-indigo-900/70 shadow-2xl flex items-center justify-between gap-2">
            <button
